...
def test_verifier_reports_not_run_without_checks() -> None:
    result = Verifier().verify(42, ())
    assert result.status is VerificationStatus.NOT_RUN
    assert result.verifier == "runtime.verifier.v1"


def test_verifier_reports_verified_only_when_all_checks_pass() -> None:
    ...
    assert result.evidence == ("value > 0", "value % 2 == 0")


def test_verifier_reports_partial_and_contains_failed_claims() -> None:
    result = Verifier().verify(
        3,
        (
...
def test_verifier_converts_check_exceptions_to_failed_claims() -> None:
...
def test_audit_recorder_is_bounded_and_returns_typed_events() -> None:
