...
def test_failed_verification_is_not_marked_complete():
    calls = []
    manager = RollbackManager(restore=lambda checkpoint: calls.append("restore") or True, verify=lambda checkpoint: False)
...
def test_rollback_is_idempotent_after_verified_success():
    calls = []
    manager = RollbackManager(restore=lambda checkpoint: calls.append("restore") or True, verify=lambda checkpoint: True)
    checkpoint = manager.checkpoint(target="/safe")
