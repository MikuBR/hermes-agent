"""Bounded, in-memory checkpoint registry for rollback metadata."""
...
class CheckpointStore:
    """Thread-safe bounded store; it never restores targets itself."""
    ...
    def fingerprint(spec: CheckpointSpec) -> str:
        material = "|".join((spec.checkpoint_id, spec.scope, spec.target, spec.created_at.isoformat(), spec.parent_id or ""))
        return sha256(material.encode("utf-8")).hexdigest()

    def put(self, spec: CheckpointSpec) -> CheckpointRecord:
        record = CheckpointRecord(spec, self.fingerprint(spec))
        with self._lock:
